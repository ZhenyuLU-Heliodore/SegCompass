...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer: PreTrainedTokenizer):
        ...
        vllm_init_kwargs, limits = {}, {}
        if config.limit_images is not None:
            limits["image"] = int(config.limit_images)
        if config.limit_videos is not None:
            limits["video"] = int(config.limit_videos)
        if limits:
            vllm_init_kwargs["limit_mm_per_prompt"] = limits
        if config.max_num_seqs is not None:
            vllm_init_kwargs["max_num_seqs"] = config.max_num_seqs
        self.inference_engine = LLM(
            model=model_path,
            skip_tokenizer_init=False,  # no need tokenizer for vllm generation
