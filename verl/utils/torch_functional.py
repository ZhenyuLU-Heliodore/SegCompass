...
def tokenize_and_postprocess_data(
    ...
    if sequence_length < max_length:
        ...
        print("Truncated tokens in a sample")
...
def compute_iou(pred_mask, gt_mask, eps=1e-7):
    p = pred_mask.to(torch.bool)
    g = gt_mask.to(torch.bool)
    inter = (p & g).sum(dim=(1, 2, 3)).float()
    union = (p | g).sum(dim=(1, 2, 3)).float()
    both_empty = (union == 0)
    iou = inter / (union + eps)
    iou = torch.where(both_empty, torch.ones_like(iou), iou)
    return iou, inter.sum(), union.sum()


def pairwise_soft_iou(pred_khw: torch.Tensor, gt_ghw: torch.Tensor) -> torch.Tensor:
    # pred: [K,H,W], gt: [G,H,W] -> iou: [K,G]
    K, H, W = pred_khw.shape
    G = gt_ghw.shape[0]
    P = pred_khw.to(torch.float32).reshape(K, -1)
    Gt = gt_ghw.to(torch.float32).reshape(G, -1)
    inter = P @ Gt.t()
    union = P.sum(-1, keepdim=True) + Gt.sum(-1).unsqueeze(0) - inter
